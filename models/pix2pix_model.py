...
class Pix2PixModel(torch.nn.Module):
    ...
    def initialize_networks(self, opt):
        ...
        if not opt.isTrain or opt.continue_train:  # True
    ...
    def compute_generator_loss(self, input_label, input_semantics, real_image, ref_label=None, ref_semantics=None, ref_image=None, self_ref=None):
        ...
        feat_loss = util.mse_loss(fake_features[self.perceptual_layer], generate_out['real_features'][self.perceptual_layer].detach())#使用了草图所对应的彩图的彩色信息做损失
        G_losses['perc'] = feat_loss * self.opt.weight_perceptual#0.001

        G_losses['contextual'] = self.get_ctx_loss(fake_features, generate_out['ref_features']) * self.opt.lambda_vgg * self.opt.ctx_w#使用了参考彩图的彩色信息做损失 ctx_w=1 测试的时候可以把这个超参数调大一些
        ...
        if self.opt.warp_mask_losstype != 'none':
            ...
            G_losses['sketch'] = (F.nll_loss(torch.log(generate_out['warp_mask'] + 1e-10), gt_label, reduce =False) * weights).sum() / (weights.sum() + 1e-5) * self.opt.weight_mask
    ...
    def generate_fake(self, input_semantics, real_image, ref_semantics=None, ref_image=None, self_ref=None):
        ...
        if self.opt.CBN_intype == 'sketch':
            ...
            CBN_in = coor_out['warp_out'] * warp_weight + input_semantics * semantic_weight
    ...
    def inference(self, input_semantics, ref_semantics=None, ref_image=None, self_ref=None):
        ...
        generate_out['fake_image'] = self.net['netG'](input_semantics,CBN_in)
        generate_out = {**generate_out, **coor_out}
