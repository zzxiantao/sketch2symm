...
def load_network(net, label, epoch, opt):
    # 添加自定义模型路径支持
    if hasattr(opt, 'pretrained_model_path') and opt.pretrained_model_path:  # False
        save_path = opt.pretrained_model_path
    else:  # True
        save_filename = '%s_net_%s.pth' % (epoch, label)  # 'latest_net_G.pth'
        save_dir = os.path.join(opt.checkpoints_dir, opt.name)  # './checkpoints/sketch2img'
        save_path = os.path.join(save_dir, save_filename)  # './checkpoints/sketch2img/latest_net_G.pth'
    
    print(f'Loading model from {save_path}')  # 'Loading model from ./checkpoints/sketch2img/latest_net_G.pth'
    
    if not os.path.exists(save_path):  # False
        print('找不到预训练模型:', save_path)
        return net
    ...
    print(f'成功加载 {len(pretrained_dict)}/{len(state_dict)} 层参数')
